...
First, load the necessary packages. More info on these packages and their installation can be found [here](introgis.qmd#loading-necessary-packages).
...
To work with the data shown in this tutorial, check out the page on [downloading raster data](BeeSpatialBound.qmd).
...
Right now we are viewing the raw numeric codes of the CDL. These codes mean something: the crop land cover class. We can tell `terra` what these codes mean so that it will show the land cover class names rather than the raw values. The information for these land cover values (along with their colors) are stored in a <a href="data/cdl_colormap.csv" download>color table downloadable here</a>.
